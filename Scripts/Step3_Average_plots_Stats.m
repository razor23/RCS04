...
fprintf('\n Printing ON files \n');
    ...
    for f=[1:3 9 10 15]
        ...
        Infile = sprintf('%s/%s/%s/%s/processed_data_gamma.mat',InDir,Groups{G},FlName,Devices{G});
        load(Infile);
...
options.color_line = rgb('Green');
    case 5
options.color_area = rgb('red');    % FOR OFF VALUES
...
plot_areaerrorbar(log10(psdout4ON{G}(:,:))',options)
xlim([0 90]);
xlabel('','Fontsize',14,'Fontname','Arial');
% ylabel('Power (log_1_0\muV^2/Hz)');
%title(sprintf('PSD Stim:ON Group: %s',Groups{G}));   %change
hold on
box off
clear psdout1ON  psdout3ON psdout4ON
